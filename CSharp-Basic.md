...
> 由于这么多年来一直用 C#摸鱼，所以基础部分只写一些自己认为比较重要的。今天看了一天书，一百来页，结果刚才发现好像都忘了...所以来开个笔记。这一部分主要介绍了什么是 `.NET` ，技术之间的关系（显然，我的理解很不透彻）。然后所C#的基础内容，变量定义、程序流控制等。
...
`Main` 函数是程序的入口点，但是在 C#9 之后，可以忽略`Main`函数直接书写代码，就像“脚本”一样：
...
所有的C#代码都应该包含在类中（9那种“脚本”编程除外。）类的声明包含 `class` 关键字，后面跟类名和一对花括号。
...
（当然，前面说过了，C#9所允许的那样脚本化的操作并不需要满足）
